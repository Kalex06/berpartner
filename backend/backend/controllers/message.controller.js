...
async function sendMessage(req, res) {
    try {
        ...
        res.status(201).json({ message: "Sikeres üzenet küldés ID:", sendMessageId });
...
async function messageAccept(req, res) {
    ...
    try {
        ...
        res.status(201).json({ message: 'Kérés elfogadva! válaszüzenet id:', savedmassage });
    }
    catch (err) {
...
async function messageReject(req, res) {
    ...
    try {
        ...
        await connection.commit();
        res.status(201).json({ message: 'Kérés elutasítva! válaszüzenet id:', savedmassage })
    }
    catch (err) {
        await connection.rollback();
...
async function sendMessageForEveryone(req,res) {
    try{
        ...
        res.status(201).json({message:`${sendedMessage} üzenet sikeresen elküldve`});

    }
    catch(err){
         res.status(500).json({ message: "Hiba az üzenetek elküldése közben!", error: err.message });
    }
}
...
module.exports = { getAllMessagesByOwner, messageAccept, messageReject, getIsOpenedCountByOwner, updateIsOpenedByOwner, sendMessage,sendMessageForEveryone };